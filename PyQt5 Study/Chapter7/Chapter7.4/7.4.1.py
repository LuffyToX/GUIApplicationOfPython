# -*- coding: utf-8 -*-
# 事件处理 重新实现事件函数、重新实现 QObject.event()
...
class MainWindow(QWidget):
    def __init__(self, parent=None):
        ...
        # QTimer.singleShot(timeout, slotFunction)
        # 在给定的时间间隔后发射此信号，调用槽函数（timeout 以毫秒计）
        # 避免窗口大小重绘事件的影响
    ...
    """ 
    重新实现其他事件，适用于 PyQt 没有提供该事件的处理函数的情况，Tab 键由
    于涉及焦点切换，不会传递给 keyPressEvent，因此，需要在这里重新定义 """
    def event(self, event):
        if event.type() == QEvent.KeyPress and event.key() == Qt.Key_Tab:
            self.key = "在 event() 中捕获 Tab 键"
            self.update()
            return True
